...
from hjcSymbol import *
from hjcVmWriter import *
from hjcXmlFile import *
from isFloat import *
...
class CompileEngine(object):
    ...
    def _CompileClassVarDec(self):
        ...
        if self.tokenizer.TokenType() == TK_KEYWORD:
            self._ExpectKeyword((KW_INT, KW_CHAR, KW_BOOLEAN, KW_FLOAT))
    ...
    def _CompileCall(self, subroutineName=None):
        ...
        ENTRY: Tokenizer positioned on the first identifier.
            If 'subroutineName' is supplied, tokenizer is on the '.'
        EXIT:  Tokenizer positioned after final ';'.
        """
        objectName = None
        ...
        if objectName == None:
            ...
            numArgs = 1
        elif self.symbolTable.KindOf(objectName) != None:
            variableKind = self.symbolTable.KindOf(objectName)
            if variableKind == None:
                self._RaiseError('identifier "%s" is undefined' % objectName)
        ...
        if objectName == None:
            objectName = self.className;
        elif self.symbolTable.KindOf(objectName) != None:
            objectName = self.symbolTable.TypeOf(objectName)
        name = '%s.%s' % (objectName, subroutineName)
        self.vmWriter.WriteCall(name, numArgs)
    ...
    def _CompileIf(self):
        ...
        if hasElse:
            skipLabel = self._UniqueLabel()
            self.vmWriter.WriteGoto(skipLabel)        
        ...
        if hasElse:
            ...
            self._CompileStatements()
            self.vmWriter.WriteLabel(skipLabel)        
    ...
    def _CompileExpression(self):
        ...
        while (self.tokenizer.TokenType() == TK_SYMBOL and \
            ...
            if operator == '+' and (tokenType == 4 or self.flag == 0 or tokenType == 2):
                self.vmWriter.WriteArithmetic(OP_ADD)
            elif operator == '+' and (tokenType == 6 or self.flag == 1 or tokenType == 2):
                self.vmWriter.WriteArithmetic(OP_FLOATADD)
            elif operator == '-' and (tokenType == 4 or self.flag == 0 or tokenType == 2):
                self.vmWriter.WriteArithmetic(OP_SUB)
            elif operator == '-' and (tokenType == 6 or self.flag == 1 or tokenType == 2):
                self.vmWriter.WriteArithmetic(OP_FLOATSUB)               
            elif operator == '*':
            ...
            elif operator == '<' and (tokenType == 4 or tokenType == 2 or self.flag == 0):
                self.vmWriter.WriteArithmetic(OP_LT)
            elif operator == '>' and (tokenType == 4 or tokenType == 2 or self.flag == 0):
                self.vmWriter.WriteArithmetic(OP_GT)
            elif operator == '<' and  (tokenType == 6 or tokenType == 2 or self.flag == 1) :
                self.vmWriter.WriteArithmetic(OP_FLOATLT)
            elif operator == '>' and (tokenType == 6 or tokenType == 2 or self.flag == 1):
                 self.vmWriter.WriteArithmetic(OP_FLOATGT)
            elif operator == '=':
    ...
    def _CompileTerm(self):
        ...
        if self.tokenizer.TokenType() == TK_INT_CONST:
        ...
        elif self.tokenizer.TokenType() == TK_FLOAT_CONST:
             self._WriteXml('floatConstant', str(self.tokenizer.FloatVal()))
             self.vmWriter.WritePush(SEG_CONST, self.tokenizer.FloatVal())
            ...
            if self.tokenizer.TokenType() == TK_SYMBOL and \
                ...
                variableType = self.symbolTable.TypeOf(variable)
