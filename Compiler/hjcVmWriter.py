...
class VmWriter(object):
    ...
    def WritePush(self, segment, index):
        ...
        if isThereDot(str(index)):
            n = len(str(index)) - 2
            self._WriteCommand(("push " + "%s " +"%." + str(n)+"f") % (self._SegName(segment), round(index, n)))
        else:
    ...
    def WritePop(self, segment, index):
        ...
        if isThereDot(str(index)):
            n = len(str(index)) - 2
            self._WriteCommand(("pop " +"%." + str(n)+"f") % round(index, n) , self._SegName(segment))
        else:
    ...
    def _OpName(self, segment):
        ...
        return ('add', 'and', 'eq', 'gt', 'lt', 'neg', 'not', 'or',
                'sub', 'floatadd', 'floatsub', 'floatgt', 'floatlt')[segment]
