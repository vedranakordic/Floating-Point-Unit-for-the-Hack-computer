class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                ...
                mybfloat = l[2].split('.')
                if len(mybfloat) == 2:
                    isdigit1 = mybfloat[0].isdigit()
                    isdigit2 = mybfloat[1].isdigit()
                    isfloat = isdigit1 and isdigit2
                
                if not l[2].isdigit() and not isfloat:
                    self._flag = False
                    Parser._error("Parser", n, "Invalid push argument")
                    return ""
                return "//" + " ".join(l) + "\n" + self._push(l[1], l[2], n, isfloat)
            else:
                self._flag = False
    ...
    def _push(self, src, loc, n, isFloat):
        if src == "constant":
            if isFloat:
                l = "@" + str(loc) + "\nD=*A\n"
            else:
